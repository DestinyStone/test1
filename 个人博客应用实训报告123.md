...
在当今移动互联网时代，智能手机已成为人们生活的重要组成部分。个人博客作为表达思想、记录生活、分享经验的重要载体，传统基于PC端的博客平台在移动场景下存在诸多不便。开发一款专为移动设备设计的个人博客应用，能够满足用户随时随地记录和分享的需求，具有重要的应用价值。
...
AddEditPostActivity实现了文章创建和编辑的统一界面，通过Intent参数区分新增和编辑模式。

Activity通过Intent获取post_id参数判断当前模式：
...
编辑模式下，调用loadPost()方法从数据库加载文章数据并填充到界面；新增模式下，显示空白表单供用户填写。

#### 2.2.6 数据保存

savePost()方法实现文章的保存逻辑，包含完整的数据校验：
